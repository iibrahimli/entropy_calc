...
int main(int argc, char* argv[]){
    ...
    unsigned long* counts;               // # of occurences of each symbol
    ...
    // initialization
    counts = malloc(sizeof *counts * n_alpha);

    // if malloc fails
    if(counts == NULL){
        fprintf(stderr, "Couldn't allocate %lu bytes of memory for 'counts'\n", n_alpha*sizeof(*counts));
        return ERR_BIG_SYMBOL_COUNT;
    }
    ...
    byte* buffer = malloc(sizeof *buffer * filesize);
    ...
    if(buffer == NULL){
        ...
        buffer = malloc(sizeof *buffer * n * buff_size);
    ...
    printf("Entropy of %15s : %.5lf\n", filename, entropy);

    double comp_rate = (double) filesize / (pow(2, entropy) * filesize / 8);
    printf("Compression rate          : %.5lf\n", comp_rate);
